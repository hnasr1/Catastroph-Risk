...
class TwoDFloodModel:
    ...
    def initialize(self):
        ...
        # flood discharge
        self.Q_flood = self.calculate_discahrge(self.return_period)
        ...
        dtm_dir =  self.engine.dataContext.sourceDataDirectory + '/dtm/dtm_final.tif'
        ...
        self.inflow_value_river_meterperhour = self.Q_flood / self.engine.regionContext.resolution**2 * 3600 / 100
    ...
    def calculate_discahrge(self, Z):
        # Q in cubic meter per second
        # Z being the return period: 2, 10, 30, 100, 300, 1000 years
    ...
    def generateSurfaceWater(self):
        ...
        # It is assumed that a flood of 20hrs occurs at peak flow ==> the inflow discharge is set to zero after 20hrs
        if self.engine.currentSimulationContext.currentTimeStep > 20:
            self.inflow_value_river_meterperhour = 0

        for iter in range(self.numIteration):

            # Adding inflow to river
            self.surfaceWater = self.surfaceWater + (
                        self.landcover == 1) * self.inflow_value_river_meterperhour * self.adjustedTimeInterval
            ...
            # Removing outflow to northeast outlet of river 2
            self.surfaceWater = self.surfaceWater - (
                        self.landcover == 2) * self.outflow_value_river_meterperhour * self.adjustedTimeInterval
